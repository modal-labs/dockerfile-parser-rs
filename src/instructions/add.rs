...
impl AddInstruction {
    pub(crate) fn from_record(record: Pair) -> Result<AddInstruction> {
        ...
        let mut destination = SpannedString {
            span: Span::new(0, 0),
            content: String::new(),
        };
        ...
        let field = inner.next().ok_or_else(|| Error::GenericParseError {
            message: "Add instruction expected a field".into(),
        })?;
...
impl<'a> TryFrom<&'a Instruction> for &'a AddInstruction {
    ...
    fn try_from(instruction: &'a Instruction) -> std::result::Result<Self, Self::Error> {
        if let Instruction::Add(c) = instruction {
            Ok(c)
        } else {
...
mod tests {
    ...
    fn add_multiple_sources() -> Result<()> {
        assert_eq!(
            parse_single("add  foo bar baz qux", Rule::add)?,
            AddInstruction {
    ...
    fn add_multiline() -> Result<()> {
        ...
        assert_eq!(parse_single("add  foo\nbar", Rule::add).is_err(), true);
    ...
    fn add_flags() -> Result<()> {
        assert_eq!(
            parse_single(
                "add  --from=alpine:3.10 /usr/lib/libssl.so.1.1 /tmp/",
                Rule::add
            )?,
            AddInstruction {
    ...
    #[test]
    fn add_boolean_flag_with_explicit_value() -> Result<()> {
        // The explicit `--link=true` / `--link=false` forms still work and the
        // value round-trips literally.
        let parsed = parse_single("add  --link=false foo bar", Rule::add)?
            .into_add()
            .unwrap();
        assert_eq!(parsed.flags.len(), 1);
        assert_eq!(parsed.flags[0].name.content, "link");
    ...
    #[test]
    fn add_boolean_flag_mixed_with_valued_flag() -> Result<()> {
        // Bare boolean flags compose with valued flags in any order.
        let parsed = parse_single("add  --link --chmod=755 foo bar", Rule::add)?
            .into_add()
            .unwrap();
    ...
    #[test]
    fn add_heredoc() -> Result<()> {
        assert_eq!(
            parse_single(
                indoc!(
                    r#"
          ADD  <<EOF /usr/share/nginx/html/index.html
          <!DOCTYPE html>
          <html>
          <head>
    ...
    #[test]
    fn add_heredoc_simple() -> Result<()> {
        assert_eq!(
            parse_single(
                indoc!(
                    r#"
          ADD  <<EOF /tmp/test.txt
          hello
          EOF
        "#
                ),
    ...
    #[test]
    fn add_heredoc_incorrect() -> Result<()> {
        assert!(parse_single(
            indoc!(
                r#"
        ADD  <<EOF /usr/share/nginx/html/index.html
    ...
    #[test]
    fn add_heredoc_with_flags() -> Result<()> {
        // Heredoc with add flags should work
        assert_eq!(
            parse_single(
                indoc!(
                    r#"
          ADD  --from=builder <<EOF /tmp/config.json
          {
            "version": "1.0",
            "env": "production"
          }
    ...
    #[test]
    fn add_heredoc_case_sensitive() -> Result<()> {
    ...
    #[test]
    fn add_heredoc_whitespace_delimiter() -> Result<()> {
        // Test that whitespace around delimiter is handled correctly
        assert_eq!(
            parse_single(
                indoc!(
                    r#"
          ADD  <<   DELIMITER   /tmp/test.txt
          some content
          DELIMITER
        "#
                ),
    ...
    #[test]
    fn add_heredoc_consecutive() -> Result<()> {
        // Test two consecutive ADD  heredoc instructions with different delimiters
        use crate::Dockerfile;

        let dockerfile_content = indoc!(
            r#"
      FROM alpine
      ADD  <<EOF1 /tmp/first.txt
      first content
      EOF1
      ADD  <<EOF2 /tmp/second.txt
      second content
      EOF2
    "#
        );
        ...
        // Check first ADD  instruction
        let first_add = dockerfile.instructions[1].clone().into_add().unwrap();
        assert_eq!(first_add.sources.len(), 1);
        match &first_add.sources[0] {
            SourceType::FileContents(content) => {
                assert_eq!(content.content, "first content\n");
            }
            _ => panic!("Expected FileContents for first ADD "),
        }
        assert_eq!(first_add.destination.content, "/tmp/first.txt");

        // Check second ADD  instruction
        let second_add = dockerfile.instructions[2].clone().into_add().unwrap();
        assert_eq!(second_add.sources.len(), 1);
        match &second_add.sources[0] {
            SourceType::FileContents(content) => {
                assert_eq!(content.content, "second content\n");
            }
            _ => panic!("Expected FileContents for second ADD "),
        }
        assert_eq!(second_add.destination.content, "/tmp/second.txt");
