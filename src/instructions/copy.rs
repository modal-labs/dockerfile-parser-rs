...
mod tests {
  ...
  fn copy_comments() -> Result<()> {
    ...
    #[test]
    fn copy_heredoc_single_quoted_delimiter() -> Result<()> {
        ...
        assert_eq!(ins.sources.len(), 1);
        match &ins.sources[0] {
            SourceType::FileContents(content) => {
                assert_eq!(content.content, "#!/bin/bash\necho hello\n");
            }
            _ => panic!("Expected FileContents"),
        }
        assert_eq!(ins.destination.content, "/test.sh");

        Ok(())
    }

    #[test]
    fn copy_heredoc_double_quoted_delimiter() -> Result<()> {
        let ins = parse_single(
            indoc!(
                r#"
          COPY <<"EOF" /test.txt
        ...
        assert_eq!(ins.destination.content, "/test.txt");

        Ok(())
    }

    #[test]
    fn copy_heredoc_quoted_in_dockerfile() -> Result<()> {
        use crate::Dockerfile;
        ...
        let copy = dockerfile.instructions[1].clone().into_copy().unwrap();
        assert_eq!(copy.sources.len(), 1);
        match &copy.sources[0] {
            SourceType::FileContents(content) => {
                assert_eq!(content.content, "#!/bin/bash\necho hello\n");
            }
            _ => panic!("Expected FileContents"),
